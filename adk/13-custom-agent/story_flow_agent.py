...
from utils import print_colorful_log
...
class StoryFlowAgent(BaseAgent):
    ...
    critic: LlmAgent
    reviser: LlmAgent
    grammar_check: LlmAgent  # Changed from grammar_checker to match parameter name
    tone_check: LlmAgent     # Changed from tone_checker to match parameter name
    loop_agent: LoopAgent = None    # Internal agent for critic-reviser loop
    sequential_agent: SequentialAgent = None  # Internal agent for post-processing

    # model_config allows setting Pydantic configurations if needed, e.g., arbitrary_types_allowed
    model_config = {"arbitrary_types_allowed": True}
    
    def __init__(self, name: str, story_generator: LlmAgent, critic: LlmAgent, reviser: LlmAgent, grammar_check: LlmAgent, tone_check: LlmAgent,):
        """
        ...
        post_processing_agent = SequentialAgent(
        ...
        # Store internal agents as instance attributes for use in _run_async_impl
        # This must be done after super().__init__() to avoid Pydantic validation errors
        self.loop_agent = refine_agent
        self.sequential_agent = post_processing_agent
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        print_colorful_log(logger, "Story state after generator", ctx.session.state.get('current_story'))
        ...
        print_colorful_log(logger, "Story state after loop", ctx.session.state.get('current_story'))
        ...
        print_colorful_log(logger, "Tone check result", tone_check_result)
